...
export class FileService implements OnApplicationBootstrap {
    ...
    async find(): Promise<File[]> {
        const res = await this.fileRepository.getAll();
        return res;
    }

    async find_notPublic(): Promise<File[]> {
        const res = await this.fileRepository.AdmingetAll();
        return res;
    }

    async find_Public(): Promise<File[]> {
        const res = await this.fileRepository.UsergetAll();
        return res;
    }

    async updatebyAdmin(id: string, dto: UpdateFileDto) {
        const res = await this.fileRepository.update(id, dto);
        return res;
    }

    async privateFile(id: string): Promise<File[]> {
        const res = await this.fileRepository.PrivateFile(id);
        return res;
    }

    async downFile(id: string) {
        const res = this.fileRepository.downloadFile(id);
        return res;
    }
}
